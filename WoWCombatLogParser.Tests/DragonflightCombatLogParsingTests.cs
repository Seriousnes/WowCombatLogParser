...
public class DragonflightCombatLogParsingTests : CombatLogParsingTestBase
{
    ...
    {
        var @event = EventGenerator.GetCombatLogEventAsync<CombatLogEvent>(input).Result;
    ...
    {
        var @event = EventGenerator.GetCombatLogEvent<DragonflightCombatantInfo>(input);
        output.WriteLine("Interesting Auras on combatant");
        foreach (var aura in @event.InterestingAuras)
        {
